...
MAN3EXT = 3pm
INSTALLDIRS = site
INSTALL_BASE = /home/symkat/perl5
DESTDIR = 
PREFIX = $(INSTALL_BASE)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Daemon/Control.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Daemon
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Daemon
...
TO_INST_PM = lib/Daemon/Control.pm

PM_TO_BLIB = lib/Daemon/Control.pm \
...
DISTVNAME = Daemon-Control-0.000007
...
manifypods : pure_all  \
	lib/Daemon/Control.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Daemon/Control.pm $(INST_MAN3DIR)/Daemon::Control.$(MAN3EXT) 
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0.000007">' > $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Daemon/Control.pm blib/lib/Daemon/Control.pm 
